...
from apps.users.models import User, LeaderProfile
...
def init_data():
    ...
    print(f'   已创建 {len(categories_data)} 个分类')
    
    print('\n5. 创建示例商品...')
    fruit_category = Category.objects.get(name='新鲜水果')
    veg_category = Category.objects.get(name='蔬菜生鲜')
    meat_category = Category.objects.get(name='肉禽蛋奶')
    ...
    if not GroupBuy.objects.filter(leader=leader, status='active').exists():
        ...
        for product in products:
            GroupBuy.objects.create(
                ...
                group_price=product.group_price * 0.9,
                min_group_size=3,
                current_people=1,
        ...
        print(f'   已创建 {len(products)} 个团购活动')
    else:
        print('   团购活动已存在')
    ...
    print('  团长: leader1 / leader123')
    print('  用户: user1 / user1234')
    print('\n管理后台地址: http://localhost:8000/admin/')
    print('API文档地址: http://localhost:8000/api/docs/swagger/')
